...
router.get("/", async (req, res) => {
  const films = await Film.find().populate("actors").populate("director").populate("genres") .populate({
    path: "comments",
    populate: {
      path: "owner",
...
router.post("/", checkAdmin, validateBody(filmAddJoi), async (req, res) => {
  try {
    ...
    if (actorSet.size < actors.length) return res.status(400).send("there's a duplicated actor")
    const actorsFound = await Cast.find({ _id: { $in: actors }, type: "Actor" })
    if (actorsFound.length < actors.length) return res.status(404).send("some of the actors isn't found")
    ...
    if (genresSet.size < genres.length) return res.status(400).send("there's a duplicated genre")
...
router.put("/:id", checkAdmin, checkId, validateBody(filmEditJoi), async (req, res) => {
  try {
    ...
    if (actors) {
      ...
      if (actorSet.size < actors.length) return res.status(400).send("there's a duplicated actor")
      const actorsFound = await Cast.find({ _id: { $in: actors }, type: "Actor" })
      if (actorsFound.length < actors.length) returnres.status(404).send("some of the actors isn't found")
    }
    if (director) {
      const directorFound = await Cast.findOne({ _id: director, type: "Director" })
      if (!directorFound) return res.status(404).send("director not found")
...
router.get("/:filmId/comments", validateId("filmId"), async (req, res) => {
  try {
    ...
    if (!film) return res.status(404).send("film is not found")
